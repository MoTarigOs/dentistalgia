import { doctors, servicesArray } from "./Constants";
import Image from '@assets/images/xray.jpg';
...
export const getDoctorDetails = (name) => {

    if(!name || name.length <= 0) return null;

    for (let i = 0; i < doctors.length; i++) {
        if(doctors[i].name.toLowerCase() === name.replaceAll('-', ' ')){
            return doctors[i];
        };
    }
